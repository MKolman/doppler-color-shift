...
/// `get_led` at `v = 0`.
pub fn get_shifted_led(lam: f64, v: f64) -> Color {
	let g = ((1.-v)/(1.+v)).sqrt();
...
pub fn get_integration_matrix(velocity: f64) -> Transformer {
	// Number of points of integration per nm
	let precision = 10;
	let fprecision = precision as f64;
	let mut result = Transformer::zeros();
