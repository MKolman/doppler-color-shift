...
pub fn get_led(lam: f64) -> ndarray::Array1<f64> {
	return array![
...
/// `get_led` at `v = 0`.
pub fn get_shifted_led(lam: f64, v: f64) -> ndarray::Array1<f64> {
	let g = ((1.-v)/(1.+v)).sqrt();
...
pub fn get_integration_matrix(velocity: f64) -> ndarray::Array2<f64> {
	// Number of points of integration per nm
	let precision = 10;
	let fprecision = precision as f64;
	let mut result: ndarray::Array2<f64> = ndarray::Array2::<f64>::zeros((3, 3));
	for lam in 350*precision..=800*precision {
		let flam = lam as f64/fprecision;
		let xyz = get_xyz(flam);
